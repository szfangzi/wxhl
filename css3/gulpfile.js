...
gulp.task('build', function () {
    ...
    var processors = [autoprefixer({ browsers: ['last 10 version'] }) //css处理里面的自动加前缀
    ];

    return gulp.src(['src/index.html', 'src/test.html']) //拿到要处理的文件
    .pipe(useref()).pipe(jsFilter).pipe(uglify()).pipe(jsFilter.restore) //把处理好的代码扔回流
    .pipe(cssFilter).pipe(postcss(processors)) //CSS处理
    .pipe(csso()).pipe(cssFilter.restore).pipe(indexHtmlFilter).pipe(rev()).pipe(indexHtmlFilter.restore).pipe(revReplace()).pipe(gulp.dest('dist'));
});
...
gulp.task('concat', function () {
    ...
    gulp.src(['src/**/jquery.min.js', 'src/js/main.js', 'src/**/*.js']).pipe(sourcemaps.init()).pipe(concat('js/all.js')).pipe(uglify()).pipe(sourcemaps.write('js')).pipe(gulp.dest('dist'));
});